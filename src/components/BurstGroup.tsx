...
function BurstGroup({ burstGroup }: BurstGroupProps) {
  ...
  const hue = topImage?._placeholderHue || 0;
  const brightness = topImage?._placeholderBrightness || 0.5;
  ...
  const durationLabel = burstGroup.durationMs > 0
    ? `${(burstGroup.durationMs / 1000).toFixed(1)}s`
    : '';
      ...
      <div className="burst-group expanded">
        <div className="burst-header">
          <div className="burst-indicator">
            <span>
              Burst ({burstGroup.frameCount})
              {fpsLabel && <span className="burst-meta"> · {fpsLabel}</span>}
              {durationLabel && <span className="burst-meta"> · {durationLabel}</span>}
            </span>
      ...
      <div className="burst-card">
        <div className="image-stack">
          ...
          <div className="image-placeholder" style={
            topImage?.thumbnailUrl
              ? { backgroundImage: `url(${topImage.thumbnailUrl})`, backgroundSize: 'cover', backgroundPosition: 'center' }
        ...
        <div className="image-info">
          <div className="filename">{topImage?.filename || 'Burst'}</div>
